!function(e) {
    ...
    e.fn.ajaxSubmit = function(t) {
        function a(a) {
            ...
            if (t.extraData)
                for (var i in t.extraData)
                    t.extraData.hasOwnProperty(i) && r.append(i, t.extraData[i]);
            t.data = null;
            ...
            t.uploadProgress && (o.xhr = function() {
                ...
                return e.upload && (e.upload.onprogress = function(e) {
                    var a = 0
                      ...
                      , n = e.total;
                    e.lengthComputable && (a = Math.ceil(r / n * 100)),
        ...
        function i(a) {
            ...
            function i() {
                ...
                var a = l.attr("target")
                  , i = l.attr("action");
                j.setAttribute("target", p),
                o || j.setAttribute("method", "POST"),
                i != f.url && j.setAttribute("action", f.url),
                f.skipEncodingOverride || o && !/post/i.test(o) || l.attr({
                ...
                try {
                    if (f.extraData)
                        for (var c in f.extraData)
                            f.extraData.hasOwnProperty(c) && u.push(e('<input type="hidden" name="' + c + '">').attr("value", f.extraData[c]).appendTo(j)[0]);
                    f.iframeTarget || (h.appendTo("body"),
                    v.attachEvent ? v.attachEvent("onload", s) : v.addEventListener("load", s, !1)),
                    setTimeout(t, 15),
                    j.submit()
                } finally {
                    j.setAttribute("action", i),
                    a ? j.setAttribute("target", a) : l.removeAttr("target"),
                    e(u).remove()
                }
            }
            function s(t) {
                if (!g.aborted && !M) {
                    try {
                        ...
                        r("cannot access response document: ", e),
                        t = w
                    }
                    if (t === k && g)
                        return void g.abort("timeout");
                    if (t == w && g)
                        return void g.abort("server abort");
                    if (E && E.location.href != f.iframeSrc || y) {
                        v.detachEvent ? v.detachEvent("onload", s) : v.removeEventListener("load", s, !1);
                        var a, i = "success";
                        try {
                            if (y)
                                throw "timeout";
                            var o = "xml" == f.dataType || E.XMLDocument || e.isXMLDoc(E);
                            if (r("isXml=" + o),
                            !o && window.opera && (null === E.body || !E.body.innerHTML) && --F)
                                return r("requeing onLoad callback, DOM not available"),
                                void setTimeout(s, 250);
                            var u = E.body ? E.body : E.documentElement;
                            g.responseText = u ? u.innerHTML : null,
                            ...
                            ,
                            u && (g.status = Number(u.getAttribute("status")) || g.status,
                        ...
                        i = null),
                        g.status && (i = g.status >= 200 && g.status < 300 || 304 === g.status ? "success" : "error"),
                        "success" === i ? (f.success && f.success.call(f.context, L, "success", g),
                        m && e.event.trigger("ajaxSuccess", [g, f])) : i && (void 0 === a && (a = g.statusText),
                        f.error && f.error.call(f.context, g, i, a),
                        m && e.event.trigger("ajaxError", [g, f, a])),
                        m && e.event.trigger("ajaxComplete", [g, f]),
                        m && !--e.active && e.event.trigger("ajaxStop"),
                        f.complete && f.complete.call(f.context, g, i),
                        M = !0,
            ...
            var u, c, f, m, p, h, v, g, x, b, y, T, j = l[0], S = !!e.fn.prop;
            if (e(":input[name=submit],:input[id=submit]", j).length)
            ...
            f.iframeTarget ? (h = e(f.iframeTarget),
            b = h.attr("name"),
            b ? p = b : h.attr("name", p)) : (h = e('<iframe name="' + p + '" src="' + f.iframeSrc + '" />'),
            h.css({
                position: "absolute",
                top: "-1000px",
                left: "-1000px"
            ...
            g = {
                ...
                abort: function(t) {
                    ...
                    g.error = a,
                    f.error && f.error.call(f.context, g, a, t),
                    m && e.event.trigger("ajaxError", [g, f, a]),
                    f.complete && f.complete.call(f.context, g, a)
                }
            },
            m = f.global,
            m && 0 === e.active++ && e.event.trigger("ajaxStart"),
            m && e.event.trigger("ajaxSend", [g, f]),
            f.beforeSend && f.beforeSend.call(f.context, g, f) === !1)
                return void (f.global && e.active--);
            if (!g.aborted) {
                x = j.clk,
                x && (b = x.name,
                b && !x.disabled && (f.extraData = f.extraData || {},
                f.extraData[b] = x.value,
                "image" == x.type && (f.extraData[b + ".x"] = j.clk_x,
                f.extraData[b + ".y"] = j.clk_y)));
                var k = 1
                  , w = 2
                  , D = e("meta[name=csrf-token]").attr("content")
                  , A = e("meta[name=csrf-param]").attr("content");
                A && D && (f.extraData = f.extraData || {},
                f.extraData[A] = D),
                f.forceSync ? i() : setTimeout(i, 10);
                var L, E, M, F = 50, X = e.parseXML || function(e, t) {
                ...
                , _ = function(t, a, r) {
                    ...
                    "string" == typeof o && ("json" === a || !a && n.indexOf("json") >= 0 ? o = O(o) : ("script" === a || !a && n.indexOf("javascript") >= 0) && e.globalEval(o)),
                    o
                }
            }
        }
        ...
        m = e.param(t.data, f)),
        t.beforeSubmit && t.beforeSubmit(p, this, t) === !1)
            return r("ajaxSubmit: submit aborted via beforeSubmit callback"),
            this;
        if (this.trigger("form-submit-validate", [p, this, t, c]),
        c.veto)
            return r("ajaxSubmit: submit vetoed via form-submit-validate trigger"),
            this;
        var h = e.param(p, f);
        m && (h = h ? h + "&" + m : m),
        "GET" == t.type.toUpperCase() ? (t.url += (t.url.indexOf("?") >= 0 ? "&" : "?") + h,
        t.data = null) : t.data = h;
        var v = [];
        ...
        !t.dataType && t.target) {
            ...
            t.success && v.push(t.success);
        t.success = function(e, a, r) {
            for (var n = t.context || t, i = 0, o = v.length; i < o; i++)
                v[i].apply(n, [e, a, r || l, l])
        }
        ;
        var x = e("input:file:enabled[value]", this)
          , b = x.length > 0
          , y = "multipart/form-data"
          , T = l.attr("enctype") == y || l.attr("encoding") == y
          , j = n.fileapi && n.formdata;
        r("fileAPI :" + j);
        var S = (b || T) && !j;
        t.iframe !== !1 && (t.iframe || S) ? t.closeKeepAlive ? e.get(t.closeKeepAlive, function() {
            i(p)
        }) : i(p) : (b || T) && j ? a(p) : e.ajax(t);
    ...
    e.fn.ajaxForm = function(n) {
        ...
        !n.delegation && 0 === this.length) {
            ...
            return !e.isReady && i.s ? (r("DOM not ready, queuing ajaxForm"),
            e(function() {
        ...
        return n.delegation ? (e(document).off("submit.form-plugin", this.selector, t).off("click.form-plugin", this.selector, a).on("submit.form-plugin", this.selector, n, t).on("click.form-plugin", this.selector, n, a),
        this) : this.ajaxFormUnbind().bind("submit.form-plugin", n, t).bind("click.form-plugin", n, a)
    }
    ,
    ...
    e.fn.formToArray = function(t, a) {
        ...
        m = o.length; s < m; s++)
            ...
            l = f.name)
                ...
                c && c.constructor == Array)
                    ...
                    null !== c && "undefined" != typeof c && (a && a.push(f),
                    r.push({
                        name: l,
                        value: c,
                        type: f.type,
        ...
        if (!t && i.clk) {
            var h = e(i.clk)
              , v = h[0];
            l = v.name,
            l && !v.disabled && "image" == v.type && (r.push({
                name: l,
                value: h.val()
            }),
            r.push({
        ...
        return this.each(function() {
            ...
            if (r) {
                ...
                if (n && n.constructor == Array)
                    ...
                    null !== n && "undefined" != typeof n && a.push({
                        name: this.name,
                        value: n
                    })
            }
    ...
    e.fieldValue = function(t, a) {
        ...
        if (void 0 === a && (a = !0),
        a && (!r || t.disabled || "reset" == n || "button" == n || ("checkbox" == n || "radio" == n) && !t.checked || ("submit" == n || "image" == n) && t.form && t.form.clk != t || "select" == i && t.selectedIndex == -1))
            return null;
        if ("select" == i) {
    ...
    e.fn.clearFields = e.fn.clearInputs = function(t) {
        ...
        return this.each(function() {
            ...
            a.test(r) || "textarea" == n ? this.value = "" : "checkbox" == r || "radio" == r ? this.checked = !1 : "select" == n ? this.selectedIndex = -1 : t && (t === !0 && /hidden/.test(r) || "string" == typeof t && e(this).is(t)) && (this.value = "")
        })
    }
    ,
